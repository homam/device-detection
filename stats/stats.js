...
(function(){
  var ref$, Obj, map, filter, find, fold, foldr, fold1, all, flatten, sum, groupBy, objToPairs, partition, join, unique, request, treefy, lowerCaseData, requestJson, getStats, getStatsBySuperCampaign, getTestStats, format;
  ...
  getStats = curry$(function(from, to, visits, country, ref, callback){
    var url;
    ...
    console.log("api << ", url);
    return requestJson(url, function(obj){
      return callback(lowerCaseData(obj));
    });
  });
  getStatsBySuperCampaign = curry$(function(from, to, visits, superCampaign, ref, callback){
    var url;
  ...
  exports.statsTree = function(fromDate, toDate, visits, country, ref, callback){
    visits == null && (visits = 0);
    country == null && (country = 0);
    ref == null && (ref = 0);
    return getStats(fromDate, toDate, visits, country, ref)(function(data){
      return callback(treefy(data));
    });
  };
  exports.statsTreeBySuperCampaign = function(fromDate, toDate, visits, superCampaign, ref, callback){
    visits == null && (visits = 0);
    superCampaign == null && (superCampaign = 0);
    ref == null && (ref = 0);
    return getStatsBySuperCampaign(fromDate, toDate, visits, superCampaign, ref)(function(data){
      return callback(treefy(data));
    });
  };
  exports.statsSummary = function(fromDate, toDate, visits, country, ref, callback){
    visits == null && (visits = 0);
    country == null && (country = 0);
    ref == null && (ref = 0);
    return getStats(fromDate, toDate, visits, country, ref)(function(data){
      return callback(format(data));
    });
  };
  ...
  exports.testsList = function(activeOnly, callback){
    return requestJson("http://mobitransapi.mozook.com/devicetestingservice.svc/json/GetTestsSummary?status=" + (activeOnly ? 'Active' : 'none'), function(raw){
      var r;
