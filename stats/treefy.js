...
(function(){
  ...
  treefy = function(){
    return function(raw){
      var data, root, emptyStats, emptyDeviceOs, emptyDeviceBrand, methods, methodStatsOrEmpty, collectStats, groupByProp, groupByBrand, groupByOs;
      data = map(function(r){
        r.children = [];
        return r;
      }, hardClone(raw));
      ...
      emptyDeviceOs = function(os){
        return {
          os: os
        };
      };
      emptyDeviceBrand = function(brand){
        return {
          brand: brand
        };
      };
      ...
      groupByProp = curry$(function(emptyMaker, propSelector, children){
        return map(function(it){
          var a;
          a = emptyMaker(it[0], it[1][0]);
          a.children = it[1];
          return a;
        })(objToPairs(groupBy(propSelector, children)));
      });
      groupByBrand = function(children){
        return groupByProp(emptyDeviceBrand, function(it){
          return it.brand;
        }, children);
      };
      groupByOs = function(children){
        return groupByProp(emptyDeviceOs, function(it){
          return it.os;
        }, children);
      };
      root.children = groupByOs(root.children);
      root.children = map(function(it){
        it.children = groupByBrand(it.children);
        return it;
      }, root.children);
