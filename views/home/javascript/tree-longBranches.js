...
(function(){
  var prelude, ref$, Obj, map, filter, each, find, fold, foldr, fold1, all, flatten, sum, groupBy, objToPairs, partition, join, unique, pow, sqrt, listOfSubscriptioMethods, formatDate, sor, shortenWurflDeviceName, exports;
  ...
  exports.treeLongBranches = function(width, height){
    ...
    updateTree = function(root, selectedStats){
      var createMethodFilter, ref$, totalVisitsSelected, totalSubscribersSelected, convAverageSelected, transition, color, nodes, links, $link, $node, $nodeEnter, $renderNodeMethodsStats;
      ...
      $node.select("text").attr("dx", function(d){
        ...
        for (i$ = 0, len$ = (ref$ = listOfSubscriptioMethods).length; i$ < len$; ++i$) {
          ...
          res$.push([m, l].concat(nodeSelectedStats(createMethodFilter([m]), d)));
        }
        dStats = res$;
