...
(function(){
  var prelude, ref$, Obj, map, filter, each, head, find, fold, foldr, fold1, all, flatten, sum, groupBy, objToPairs, partition, join, unique, sortBy, reverse, listOfSubscriptionMethods, formatDate, pow, pow2, sqrt, treeUiTypes, treeChart, showDialog, showCreateATestDialog, showConcludeATestDialog;
  prelude = require('prelude-ls');
  ref$ = require('prelude-ls'), Obj = ref$.Obj, map = ref$.map, filter = ref$.filter, each = ref$.each, head = ref$.head, find = ref$.find, fold = ref$.fold, foldr = ref$.foldr, fold1 = ref$.fold1, all = ref$.all, flatten = ref$.flatten, sum = ref$.sum, groupBy = ref$.groupBy, objToPairs = ref$.objToPairs, partition = ref$.partition, join = ref$.join, unique = ref$.unique, sortBy = ref$.sortBy, reverse = ref$.reverse;
  ...
  $(function(){
    ...
    updateStatsAtFooter = function(node){
      var allMethodsSummary, x$, $summarySpan, $li, $liEnter, renderMethodStats;
      ...
      x$ = $summarySpan = d3.select('.all-methods-summary').selectAll('span').data(objToPairs(allMethodsSummary));
      x$.enter().append('span').attr('class', function(it){
        return it[0];
      });
      x$.text(function(it){
  ...
  showConcludeATestDialog = function(node){
    var makeStat, dialog, stats, testId, $dialog, render;
    ...
    stats = sortBy(function(it){
      var ref$;
      return it.subscribers / ((ref$ = it.visits) != null ? ref$ : 1);
    }, node.stats);
    stats = insertAfter(function(it){
      return it.method === 'BANNER_JAVAAPP';
    }, head(makeStat('JAVA_APP', 1, 1)), stats);
    ...
    render = function(){
      ...
      $liEnter.append('span').attr('class', 'close').text('x').on('click', function(d){
        ...
        stats = filter(function(it){
          return !!it && it.method !== method;
        }, stats);
        return render();
      });
      $li.select('span.close').attr('data-method', function(it){
        return it.method;
      });
