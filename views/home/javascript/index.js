...
(function(){
  var sumStats, visits, subscribers, foldRealNodes, updateAllNodes, killChildren, prelude, map, flatten, groupBy, objToPairs, fold1, fold, filter, pow, sqrt, width, height, tree, diagonal, svg;
  ...
  killChildren = curry$(function(minVisits, node){
    switch (false) {
    case node.children.length !== 0:
      node;
      break;
    default:
      node.children = filter(function(it){
        return it.visits > minVisits;
      }, node.children);
      map(killChildren(minVisits), node.children);
    }
    return node;
  });
  ...
  $.get('/data/ae.json', function(root){
    var selectedSubscriptionMethods, statsFilter, totalVisits, totalSubscribers, totalConv, convStnDev, convAverage, sor, color, nodes, links, link, node;
    selectedSubscriptionMethods = ['sms', 'smsto', 'mailto', 'JAVA_APP'];
    statsFilter = function(node){
      var i$, ref$, len$, m, results$ = [];
      for (i$ = 0, len$ = (ref$ = node.stats).length; i$ < len$; ++i$) {
        m = ref$[i$];
        if (in$(m.method, selectedSubscriptionMethods)) {
          results$.push(m);
        }
      }
      return results$;
